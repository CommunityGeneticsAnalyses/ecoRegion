...
f.stay <- gsub('Current','2010',f.stay)
...
mu$mu <- mu$mu[c(3,1,2,6,4,5,9,7,8),]
xlim <- c(-1.35,2.5);ylim <- c(-2.35,2)

### Fig 1. Ordination of ecoregions moving through climate space
pdf('../results/EcoReg_FigA.pdf')
sc <- chPlot(min.stay,f=f.stay,col=ord.alpha,pch=ord.pch,xlim=c(-1,1.25),ylim=c(-1,0.5),return.coord=TRUE,se=FALSE,cex=1.25)
stay.lab <- rownames(sc[[1]][grepl('UHP',rownames(sc[[1]])),])
stay.lab <- do.call(rbind,strsplit(stay.lab,' '))[,2]
stay.lab <- gsub('2010','Current',stay.lab)
plot(vec.stay,col=grey(0.75))
text(c(-0.25,-0.25,0.65),c(-0.30,0.25,0.30),labels=c('CCV','SD','UHP'),col=c(2,1,3))
chArrow(sc)
text(sc[[1]][grepl('UHP',rownames(sc[[1]])),1:2],labels=stay.lab,pos=3,col='darkgrey')
...
sc.m <- chPlot(move.all[,2:3],f=f,col=move.alpha,pch=move.pch,xlim=xlim,ylim=ylim,cex=1.25,plot.axes=FALSE,xlab='Longitude (decimal)',ylab='Latitude (decimal)',return.coord=TRUE,se=FALSE)
move.lab <- rownames(sc.m[[1]][grepl('UHP',rownames(sc.m[[1]])),])
move.lab <- do.call(rbind,strsplit(move.lab,' '))[,2]
move.lab <- gsub('2010','Current',move.lab)
plot(vec.move,col=grey(0.75))
...
chArrow(mu)
text(sc.m[[1]][grepl('UHP',rownames(sc.m[[1]])),1:2],labels=move.lab,pos=3,col='darkgrey')
